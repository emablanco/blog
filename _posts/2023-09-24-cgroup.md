...
```
A tener en cuenta: en el archivo **_/etc/systemd/system/user@.service.d/delegate.conf_** no se necesita agregar **WantedBy=multi-user.target**, ya que los servicios dentro de un **slice** de usuario se ejecutarán en las sesiones de usuario y no en el arranque del sistema.
La activación del **slice** se gestionará automáticamente cuando un usuario inicie sesión y finalizara cuando el usuario cierre sesión.
